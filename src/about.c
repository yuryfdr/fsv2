...
draw_text( void )
{
	...
	text_draw_straight( _("fsv2 - 3D File System Visualizer"), &tpos, &tdims );
	...
	if (about_part > 0.75) {
		...
		q = SQR(SQR(p));
		glColor3f( q, q, 0.0 );
		text_draw_straight( "http://www.fedorchenko.net/fsv2.php", &tpos, &tdims );
		tpos.y = -125.0;
		text_draw_straight( "___________________________________", &tpos, &tdims );
	}
